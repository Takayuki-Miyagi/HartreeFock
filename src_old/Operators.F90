...
contains
  ...
  subroutine InitOp(this, jr, pr, zr, optr, ms, is_three)
    ...
    this%ms => ms
    this%zero = 0.d0
    call this%one%init(ms, this%Scalar, optr, jr, pr, zr)
    call this%two%init(ms, this%Scalar, optr, jr, pr, zr)
  ...
  subroutine SetOperatorFromFile(this, file_nn, file_3n, &
    ...
    class(Op), intent(inout), target :: this
  ...
  subroutine NormalOrdering(this) ! NO2B approx
    class(Op), intent(inout), target :: this
    ...
    if(this%is_three_body) then
#ifdef single_precision
      op2from3 = this%thr21%NormalOrderingFromSp3To2()
#else
      op2from3 = this%thr21%NormalOrderingFrom3To2()
#endif
    end if

    op1from3 = op2from3%NormalOrderingFrom2To1()
    op1from2 = this%two%NormalOrderingFrom2To1()
  ...
  subroutine UnNormalOrdering(this) ! From NO2B approx. Op
    class(Op), intent(inout), target :: this
  ...
  subroutine NO2BApproximation(this)
    class(Op), intent(inout), target :: this
    if(.not. this%is_three_body) then
      write(*,'(a)') "This operator does not include three-body part."
